...
import { Car } from '../cars/schemas/car.schema'; // 🔽 NEW
...
export class ReservationsService {
  constructor(
    ...
    @InjectModel(Car.name) private carModel: Model<any>,
    private readonly repairBaysService: RepairBaysService, // ✅ Injection ajoutée
    @Inject(forwardRef(() => MaintenancesService)) // 🔽 Use forwardRef
    private readonly maintenancesService: MaintenancesService, // 🔽 NEW
  ) {}
  ...
  async create(createDto: CreateReservationDto, currentUser: any) {
    ...
    // 🔽 NEW: Validate car exists and belongs to user
    const car = await this.carModel.findById(createDto.carId);
    if (!car) {
    ...
    const reservationData: any = {
      ...
      carId: new Types.ObjectId(createDto.carId), // 🔽 NEW
      garageId: new Types.ObjectId(createDto.garageId),
    ...
    return await this.reservationModel.findById(saved._id)
      ...
      .populate('carId', 'marque modele annee immatriculation imageUrl')
  ...
  async updateStatus(id: string, status: string, userId: string) {
    ...
    // ✅ When confirming a reservation: confirm via RepairBaysService + create maintenance
    if (status === 'confirmé' && reservation.status === 'en_attente') {
      await this.repairBaysService.confirmReservation(id);

      // Re-fetch populated reservation (including car + garage)
      const updated = await this.reservationModel.findById(id)
        .populate('userId', 'nom prenom email')
        .populate('garageId', 'nom adresse')
        .populate('carId', 'marque modele immatriculation')
        .exec();

      if (!updated) {
        throw new NotFoundException('Réservation non trouvée après confirmation');
      }

      // 🔽 Build maintenance from reservation
      const reservationDate = updated.date;
      const maintenanceTitle = `Entretien - ${updated.services.join(', ') || 'Réservation garage'}`;

      const maintenanceType: 'vidange' | 'révision' | 'réparation' = (() => {
        if (updated.services.some(s => s.includes('vidange'))) return 'vidange';
        if (updated.services.some(s => ['révision complète', 'diagnostic électronique'].includes(s))) {
          return 'révision';
        }
        return 'réparation';
      })();

      const createMaintenanceDto: any = {
        title: maintenanceTitle,
        notes: updated.commentaires || undefined,
        tags: updated.services && updated.services.length > 0 ? updated.services : ['reservation'],
        status: 'planned',
        dueAt: reservationDate,
        mileage: undefined,
        type: maintenanceType,
        date: reservationDate,
        cout: updated.totalAmount || 0,
        garage: updated.garageId._id.toString(),
        voiture: (updated as any).carId._id.toString(),
      };
      ...
      try {
        ...
        console.log('✅ Maintenance créée automatiquement pour la réservation confirmée');
      } catch (e) {
        console.error('❌ Erreur lors de la création de l\'entretien:', e);
      }
