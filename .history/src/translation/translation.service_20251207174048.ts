...
export class TranslationService {
  ...
  // Request deduplication cache (short-term, 100ms)
  private pendingRequests: Map<string, Promise<string[]>> = new Map();
  private readonly DEDUP_CACHE_DURATION = 100; // milliseconds
   ...
   * Implements request deduplication to prevent duplicate API calls within 100ms
   *
  ...
  async translateText(
    ...
    if (pendingRequest) {
      this.logger.debug('Deduplicating identical translation request');
    ...
    }, this.DEDUP_CACHE_DURATION);

    return requestPromise;
  }

  /**
   * Perform actual translation (called by translateText after deduplication check)
   */
